        ...
        <form method="POST" action="/pcmove/create">
            ...
            <fieldset class="col-6 mx-auto">
                <div class="form-group row">
                    ...
                    <input type="text" name="from_name" class="form-control col-8" value="{{ old('from_name') }}" required>
                </div>
                <div class="form-group row">
                    <label for="to_name" class="col-3 col-form-label">To User:</label>
                    <input type="text" name="to_name" class="form-control col-8" value="{{ old('to_name') }}" required>
                </div>
                <div class="form-group row">
                    <label for="from_site" class="col-3 col-form-label">From Site:</label>
                    <select class="form-control col-8" name="from_site">
                        @foreach($siteNames as $siteName)
                            <option value="{{$siteName->siteName}}" {{ old("from_site") == $siteName->siteName ? "selected" : "" }}>{{$siteName->siteName}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group row">
                    ...
                    <select class="form-control col-8" name="to_site">
                        @foreach($siteNames as $siteName)
                            <option value="{{$siteName->siteName}}" {{ old("to_site") == $siteName->siteName ? "selected" : "" }}>{{$siteName->siteName}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group row">
                    <label for="move_date" class="col-3 col-form-label">Date of Move:</label>
                    <input type="date" name="move_date" class="form-control col-8" value="{{ old('move_date') }}">
                </div>
                <div class="form-group row">
                    <label for="contact_details" class="col-3 col-form-label">Contact Details:</label>
                    <input type="text" name="contact_details" class="form-control col-8" value="{{ old('contact_details') }}" placeholder="Email address or Phone Number">
                </div>
